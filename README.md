...
(Renderの無料版のため、最初は画面がAPPLICATION LOADINGとなり、時間がかかります)

## 概要
- ユーザーが投票を行い、その結果をリアルタイムで集計・表示するためのWebツール
- 投票データはMongoDBに保存され、管理画面から簡単に操作

## 主な機能

1. **投票機能**
   - ユーザーは名前を入力し、複数回に分けて数字を選択
   - 同じ名前で再投票を行うと、既存のデータが上書き

2. **リアルタイム集計**
   - 投票データをリアルタイムで集計し、ポイントを計算
   - ポイントはカスタマイズ可能で、順位ごとに異なる値を設定
